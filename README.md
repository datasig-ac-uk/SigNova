...
* pysegments:
  * signal_tolerance: $2^{sig\underscore tol}$ governs how much we split intervals in order to find an interval where the characteristic function is True. For example, if $sig\_tol$=3 we have $2^{sig\_tol}$=8, we will never go finer than 8.).
